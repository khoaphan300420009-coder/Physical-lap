...
const SolarSim: React.FC = () => {
    const [state, setState] = useState<SimState>({
        ...
        scaleMode: 'Visual', viewCenter: 'sun',
    ...
    const [planets] = useState<PlanetConfig[]>(PLANETS_DATA || []);
    ...
    useEffect(() => {
        ...
        const render = () => {
            ...
            if (!state.paused) setState(s => ({ ...s, time: s.time + s.speed * 0.1 }));

            const planetPos = planets.filter(p => p).map(p => ({...p, ...calculatePosition(p, state.time)}));
            
            let viewX = 0, viewY = 0;
            if (state.viewCenter !== 'sun') {
                const target = planetPos.find(p => p.id === state.viewCenter);
            ...
            for(let i=0; i<200; i++) {
                ...
                const sy = (Math.cos(i*123) * h) - viewY * 0.05 * Math.cos(state.tilt);
            ...
            if (state.showOrbits) {
                planetPos.forEach(p => {
                    ...
                    ctx.ellipse(-viewX, -viewY * Math.cos(state.tilt), r, r * Math.cos(state.tilt), 0, 0, Math.PI*2);
            ...
            if (state.showAsteroids) {
                ...
                asteroids.forEach((ast, i) => {
                    const currAngle = ast.angle + ast.speed * state.time * 0.01;
                    let scale = ast.dist * 10;
                    if (state.scaleMode === 'Visual') scale = Math.pow(ast.dist, 0.4) * 80;

                    const x = Math.cos(currAngle) * scale - viewX;
                    const y = (Math.sin(currAngle) * scale - viewY) * Math.cos(state.tilt) + ast.height * 50 * Math.sin(state.tilt);
            ...
            // Comet
            if (state.showComet) {
                const cometPos = calculateCometPos(state.time);
                const cx_scr = cometPos.x - viewX;
                const cy_scr = (cometPos.y - viewY) * Math.cos(state.tilt);
                ...
                const gradient = ctx.createLinearGradient(cx_scr, cy_scr, cx_scr + (dx/len)*tailLen, cy_scr + (dy/len)*tailLen * Math.cos(state.tilt));
                gradient.addColorStop(0, 'rgba(56, 189, 248, 0.8)');
                gradient.addColorStop(1, 'transparent');
                
                ctx.lineWidth = 4/camera.zoom;
                ctx.strokeStyle = gradient;
                ctx.beginPath(); ctx.moveTo(cx_scr, cy_scr); ctx.lineTo(cx_scr + (dx/len)*tailLen, cy_scr + (dy/len)*tailLen * Math.cos(state.tilt)); ctx.stroke();
            ...
            const sortedPlanets = [...planetPos].sort((a, b) => {
                 const ay = (a.y - viewY) * Math.cos(state.tilt);
                 const by = (b.y - viewY) * Math.cos(state.tilt);
                 return ay - by;
            });
            ...
            sortedPlanets.forEach(p => {
                ...
                const screenY = (p.y - viewY) * Math.cos(state.tilt);
                
                let radius = state.scaleMode === 'Visual' ? Math.max(3, Math.log(p.radiusKM)*1.5) : p.radiusKM / 1000;
                ...
                if (p.id === 'sun') {
                    ...
                    if (p.id === 'jupiter') {
                        ...
                        // Continents (approximated)
                        ctx.save();
                        ctx.clip();
                        ctx.rotate(state.time * 0.1); // Spin
                    ...
                    // Rings (Saturn/Uranus)
                    if (state.showRings && (p.id === 'saturn' || p.id === 'uranus')) {
                        ...
                        const ry = rx * 0.3 * Math.cos(state.tilt); 
    ...
    }, [state, camera, asteroids, planets]);
    ...
    currentDate.setDate(currentDate.getDate() + state.time);
    const dateStr = currentDate.toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' });

    const selectedInfo = state.selectedPlanet ? planets.find(p => p && p.id === state.selectedPlanet) : null;
        ...
        <div className="flex flex-col h-full w-full bg-[#020408] text-white font-sans overflow-hidden relative group/canvas">
            ...
            <div className="flex-1 relative overflow-hidden">
                ...
                <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-start bg-gradient-to-b from-black/80 to-transparent pointer-events-none z-20">
                    <div className="pointer-events-auto flex flex-col gap-3">
                        <div className="flex items-center gap-4 bg-white/10 backdrop-blur-xl px-5 py-3 rounded-2xl border border-white/10 shadow-xl">
                            <button onClick={() => setState(s => ({...s, paused: !s.paused}))} className="w-10 h-10 flex items-center justify-center bg-cyan-600 rounded-xl hover:bg-cyan-500 transition-colors shadow-lg shadow-cyan-500/20">
                        ...
                        <div className="bg-black/60 backdrop-blur px-4 py-2 rounded-xl border border-white/5 flex items-center gap-3 w-fit">
                            ...
                            <input type="range" min="0" max="100" value={state.speed} onChange={e => setState(s => ({...s, speed: Number(e.target.value)}))} className="w-32 accent-cyan-500 h-1 bg-white/20 rounded cursor-pointer"/>
                            <span className="text-xs font-mono text-cyan-400">{state.speed.toFixed(1)}x</span>
                        </div>
                    ...
                    <div className="pointer-events-auto flex flex-col gap-2 items-end">
                        <button onClick={() => setState(s => ({...s, tilt: s.tilt === 0 ? 0.8 : 0}))} className="p-3 bg-white/10 hover:bg-white/20 rounded-xl border border-white/10 transition-all text-white" title="Toggle 2D/3D">
                            <Layers size={20}/>
                        </button>
                        
                         <button onClick={() => setState(s => ({...s, showRings: !s.showRings}))} className={`p-3 rounded-xl border transition-all text-white ${state.showRings ? 'bg-purple-500/30 border-purple-400' : 'bg-white/10 border-white/10'}`} title="Planetary Rings">
                        ...
                        <div className="bg-black/60 px-3 py-1 rounded-lg text-[10px] text-slate-400 font-bold uppercase tracking-widest border border-white/5">
                            {state.tilt === 0 ? 'Top-Down View' : 'Orbital View (3D)'}
                        </div>
                    </div>
                </div>
                ...
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-4 pointer-events-auto z-20">
                    <div className="flex bg-white/10 backdrop-blur-xl p-1.5 rounded-2xl border border-white/10 shadow-2xl">
                        <button onClick={() => setState(s => ({...s, scaleMode: 'Visual'}))} className={`px-6 py-2 rounded-xl text-xs font-bold transition-all ${state.scaleMode==='Visual' ? 'bg-cyan-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>Visual Scale</button>
                        <button onClick={() => setState(s => ({...s, scaleMode: 'Real'}))} className={`px-6 py-2 rounded-xl text-xs font-bold transition-all ${state.scaleMode==='Real' ? 'bg-cyan-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>Real Scale</button>
                    </div>
                </div>
                ...
                {selectedInfo && (
                    <div className="absolute top-24 right-6 w-80 bg-[#0a0f18]/90 backdrop-blur-2xl border border-cyan-500/30 p-6 rounded-[2rem] shadow-[0_0_50px_rgba(0,0,0,0.5)] animate-[slideInRight_0.3s] z-30">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button onClick={() => setState(s => ({...s, selectedPlanet: null}))} className="p-2 hover:bg-white/10 rounded-full"><Info size={20} className="text-cyan-400"/></button>
                        </div>
            ...
            <div className="h-32 bg-[#0a0f18]/80 backdrop-blur-xl border-t border-white/10 shrink-0 flex flex-col justify-center px-6 relative z-30">
                 <div className="flex items-center gap-4 overflow-x-auto custom-scroll pb-2">
                    <button 
                         onClick={() => setState(s => ({...s, viewCenter: 'sun', selectedPlanet: 'sun'}))}
                         className={`flex flex-col items-center gap-2 p-3 rounded-xl transition-all min-w-[80px] ${state.viewCenter === 'sun' ? 'bg-white/10 border border-white/20' : 'hover:bg-white/5 opacity-70 hover:opacity-100'}`}
                    >
                    ...
                    {planets.filter(p => p && p.id !== 'sun').map(p => (
                        <button 
                            key={p.id} 
                            onClick={() => setState(s => ({...s, viewCenter: p.id, selectedPlanet: p.id}))}
                            className={`flex flex-col items-center gap-2 p-3 rounded-xl transition-all min-w-[80px] ${state.viewCenter === p.id ? 'bg-white/10 border border-white/20' : 'hover:bg-white/5 opacity-70 hover:opacity-100'}`}
                        >
